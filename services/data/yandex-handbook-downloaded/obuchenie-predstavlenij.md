...
[Источник](https://youtu.be/cWzi38-vDbE?t=312)
...
[Источник](https://arxiv.org/abs/1311.2901)
...
[Источник](https://huggingface.co/transformers/main_classes/optimizer_schedules.html)
...
[Источник](https://openaccess.thecvf.com/content_iccv_2015/papers/Babenko_Aggregating_Local_Deep_ICCV_2015_paper.pdf)
...
Если будем только притягивать, то любой константный вектор будет хорошим решением. К примеру .
...
[Источник](https://generallyintelligent.ai/understanding-self-supervised contrastive-learning.html)
...
1. Семплируем мини-батч объектов ;
2. Для каждого объекта в минибатче :
— Cемплируем две аугментации ;

— Вычисляем эмбединги ;

— Вычисляем проекции ;
3. Вычисляем contrastive loss , используя .
...
4. Делаем шаг по градиенту , повторяем с шага 1 пока не сойдёмся;
5. Используем для генерации эмбедингов или файнтюнинга под supervised задачу.
...
3. Предсказание патча высокого разрешения разрешение с использованием -лосса.
