...
где . Тогда из доказанного выше результата следует (см. Лемму 3 из [статьи С. Стиха](https://arxiv.org/pdf/1907.04232.pdf)), что после итераций
...
Однако важно отметить, что оценка на достаточно точно отражает поведение метода вблизи решения: даже если точка оказалась по какой-то причине близка к решению (или даже просто совпала с решением), то и, в частности, будут порядка или . Следовательно, при следующем шаге метод с большой вероятностью отдалится от/выйдет из решения, поскольку или .
