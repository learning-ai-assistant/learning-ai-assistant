...
В [этой статье](http://www.opt-ml.org/papers/OPT2016_paper_24.pdf) было теоретически обосновано, что если параметр частый, но нерелевантный и абсолютно шумный, то дисперсия будет иметь асимптотику . Из этого следует, что, если сделать регуляризацию порядка , мы лишим такой случайный шум почти любых шансов выйти из нуля.
...
1. В модели параметрам нужно запомнить «отклонение» от среднего ;
2. В модели параметрам нужно запомнить абсолютное значение предсказания.
...
1. Коэффициенты и нетривиальным образом взаимодействуют. Это крайне неудобно при переборе гиперпараметров: изменение learning rate должно влечь за собой переподбор коэффициента регуляризации по полной сетке;
2. В квадратах градиентов мы хотим видеть только адаптивность к кривизне самой функции , но теперь там ещё добавка .
