...
Модель скользящего среднего порядка или просто MA() предполагает следующую зависимость данных:
...
Пусть гиперпараметры фиксированы.&tab;В предположении, что — гауссовский белый шум, в нашей модели мы можем выписать функцию правдоподобия где — соместная плотность. Из-за того, что имеют нормальное распределение, она будет иметь разумный вид. Соответственно, в качестве оценок параметров берется оценка максимального правдоподобия.
...
4. Выбор пилотных и по PACF и ACF.
5. Вокруг этих параметров подбираем оптим. модель по /.
