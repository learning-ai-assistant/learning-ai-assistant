...
![mnist](https://yastatic.net/s3/education-portal/media/mnist_digits_73edc82188_86cd92299c.webp)
...
![counterfeiter](https://yastatic.net/s3/education-portal/media/counterfeiter_analogy_8df968ecec_a40b7b9c4a_0cc3c5a1fe.svg)
...
1. Мы замеряем, насколько реалистичными являются сгенерированные сэмплы , используя для этого оптимальный дискриминатор .
2. Мы хотим увеличить отклик дискриминатора на каждом сэмпле, т.е. пытаемся модифицировать каждый предсказанный элемент так, чтобы на нём стало выше значение .
...
![Artboard](https://yastatic.net/s3/education-portal/media/Artboard_33_f123d6e9af_bb7a95aff1.svg)
...
1. Решить внутреннюю задачу максимизации по , повторяя шаги ниже до сходимости по параметрам дискриминатора к оптимальному значению :
...
2. Сделать шаг SGD для внешней задачи минимизации по :
...
![Artboard](https://yastatic.net/s3/education-portal/media/Artboard_35_00b6473069_48b468655d.svg)
...
![Artboard](https://yastatic.net/s3/education-portal/media/Artboard_34_1831bed686_107e48ea9e.svg)
...
Одним из тестов на подобное поведение является интерполяция между сгенерированными примерами. Возьмём два случайных вектора и из . Рассмотрим все векторы, которые лежат между ними . К каждому такому вектору применим наш генератор и получим для промежуточных векторов и для и . Для правильно обученного GANа мы должны увидеть следующую картинку: при изменении коэффициента изображение должно плавно меняться и перетекать из в . При этом каждая промежуточная картинка должна быть так же реалистичным сэмплом.

![](https://yastatic.net/s3/education-portal/media/interpolation_example_f6ce073e6a_dcaef51a6c.webp)

[ссылка на источник картинки](https://arxiv.org/abs/1707.05776)

Качество такой интерполяции сложно измерить численно, но если мы видим, что промежуточные результаты меняются случайно без какой-либо связи с семантикой интерполируемых примеров, то это говорит о плохом качестве генератора. Стоит упомянуть, что для сэмплов из нормального распределения, которое обычно имеют векторы , намного лучше работает [интерполяция по сфере (Slerp)](https://en.wikipedia.org/wiki/Slerp), потому что в многомерном пространстве векторы практически всегда будут лежать в объёме вокруг сферы диаметра , где – размерность вектора .
...
![](https://yastatic.net/s3/education-portal/media/latent_directions_d708e8af59_56a407218b.webp)

[ссылка на источник картинки](http://proceedings.mlr.press/v119/voynov20a/voynov20a.pdf)
...
![](https://yastatic.net/s3/education-portal/media/dcgan_d8eaa0ce24_237f87ff39.webp)


Модель DCGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1511.06434.pdf)
...
![](https://yastatic.net/s3/education-portal/media/dcgan_samples_6b57300506_43b4abf043.webp)


Результат работы DCGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1511.06434.pdf)
...
![](https://yastatic.net/s3/education-portal/media/dcgan_interpolation_879fe2b998_0971bf6eda.webp)


Так выглядит интерполяция в скрытом пространстве для модели DCGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1511.06434.pdf)
...
Далее будем обозначать условие как . Наша задача — построить генератор, который бы моделировал .
...
![](https://yastatic.net/s3/education-portal/media/cgan_1b1ac03e48_4cf45b5693.webp)


Модель Conditional GAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1411.1784.pdf)
...
![](https://yastatic.net/s3/education-portal/media/pggan_1_9d2b69beca_515c4fedba.webp)


Иллюстрация работы генератора и дискриминатора в модели Progressive Growing GANs. [Ссылка на источник картинки.](https://arxiv.org/pdf/1710.10196.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_1_e8931c250a_1a842b0e64.webp)


Слева: традиционный генератор. Справа: генератор модели StyleGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1812.04948.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_examples_f18ca2f78e_4336874805.webp)


Примеры генерации StyleGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1812.04948.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_artifacts_e8c61c9be6_92dbafc75f.webp)


Примеры артефактов StyleGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1912.04958.pdf)
...
2. Нормализацию из предыдущего пункта заменили на демодуляцию весов. По сути это та же нормализация, только теперь нормализуются веса свёрток, а не входные данные (см рисунок ниже, (d) Weight demodulation. Обратите внимание на ).

![](https://yastatic.net/s3/education-portal/media/stylegan2_605de023ae_b010b216d2.webp)


Изменения, которые добавили в StyleGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1912.04958.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan2_progressive_d01545db47_9ec9aa0376.webp)


Артефакты из-за прогрессивной генерации в StyleGAN. [Ссылка на источник картинки.](https://arxiv.org/pdf/1912.04958.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan2_results_cb58a8cc0b_f15cc11383.webp)


Примеры изображений, сгенерированных с помощью StyleGAN2. [Ссылка на источник картинки.](https://arxiv.org/pdf/1912.04958.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_ada_augmentations_019b3516fe_9a8260a37a.webp)


На левом рисунке (b) изображено, куда добавляется аугментация (синие блоки). На правом рисунке (c) изображена степень аугментации в зависимости от контролирующего её параметра p. [Ссылка на источник картинки.](https://arxiv.org/pdf/2006.06676.pdf)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_t_architecture_c881b0cf15_989f0c2419.webp)


Архитектура модели StyleGAN-T. [Ссылка на источник картинки.](https://arxiv.org/abs/2301.09515)
...
![](https://yastatic.net/s3/education-portal/media/stylegan_t_results_f7fd7e01d0_8cade2358c.webp)


Результаты генерации моделью StyleGAN-T. [Ссылка на источник картинки.](https://arxiv.org/abs/2301.09515)
...
![](https://yastatic.net/s3/education-portal/media/inpainting_example_bd2b238b82_ee4a87ad29.webp)


Пример работы модели инпеинтинга. [Ссылка на источник картинки.](https://arxiv.org/abs/2109.07161)
...
![](https://yastatic.net/s3/education-portal/media/outpainting_example_5ca5da2a68_d418dcd028.webp)


Пример outpainting, сделанный моделью In&Out. [Ссылка на источник картинки.](https://arxiv.org/abs/2104.00675)
...
Пример работы Drag Your GAN.[cсылка на источник изображения](https://vcai.mpi-inf.mpg.de/projects/DragGAN/)
