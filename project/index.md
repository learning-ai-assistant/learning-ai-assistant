...
Используй `project/`, чтобы понять repository boundaries, technical baseline и top-level navigation. Этот слой не является engineering Source of Truth для архитектуры.
...
7. [`context-migration.md`](context-migration.md) для migration history и зафиксированного target state.

## Границы слоя

- `project/` содержит durable repository context.
- `docs/` содержит architecture, contracts и другие engineering SoT artifacts.
- `operational_scope/` является execution layer.
